...
- ASSIGN: share contact details between customer and technician AFTER payment is confirmed. This is a POST-PAYMENT action and is completely different from FORWARD_QUOTE (which happens BEFORE payment). e.g. "assign #X", "send details for X", "X ke details bhej do", "please connect both for #X", "connect both for #X", "connect both parties for #X", "connect them for #X", "share details for #X", "share both details for #X", "send details to both for #X", "please share details for #X", "connect customer and technician for #X", "link both for #X", "send contacts for #X", "share contacts for #X", "please connect for #X", "connect for #X". ANY message containing "connect", "share details", "share contacts", "send contacts", "link both", or "send details to both" together with a job reference is ALWAYS ASSIGN — never FORWARD_QUOTE.
