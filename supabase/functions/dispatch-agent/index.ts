...
async function sendSMS(supabase: any, tech: Tech, job: any) {
  ...
  const msg = `Tyrefly job: ${job.issue_type} in ${job.postcode}.${wheelsTxt}${sizeTxt} ${blurb}${ask} IMPORTANT: For THIS job you MUST share a fresh 📍live location pin right now (tap 📎 → Location → Share live location, 8 hours) — quotes without an up-to-date pin for this job will not be accepted. Job#${job.id.slice(0, 6)}`;
