// Ported from the admin-send-quote edge function.
// Same DB updates, same Stripe checkout, same customer WhatsApp message.
import { isCustomerQuoteAmountValid, normalizeSuspiciousQuotePrice } from "@/lib/quote-price";
import { resolveQuoteLocationForAllocation } from "@/lib/quote-location";
import { createCheckoutSession } from "./stripe.server";
import { sendMessage, serviceClient, shortenUrl } from "./ops.server";

export async function runAdminSendQuote(input: {
  job_id: string;
  quote_id?: string | undefined;
}): Promise<{ ok: true; price: number }> {
  const { job_id, quote_id } = input;
  ...
  quoteQuery = quote_id
    ...
    : quoteQuery.in("status", ["pending", "accepted"]);
  ...
  if (!isCustomerQuoteAmountValid(mergedPrice)) {
    throw new Error(
      `Quote for job #${shortRef} has an invalid amount (£${quoteRow.price_gbp ?? "—"}). Ask the technician to resend the price in pounds before sending it to the customer.`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Could not generate the payment link for job #${shortRef}. The quote was not sent to the customer.`,
    );
  }
  ...
  await supabase
    .from("jobs")
    .update({ status: "awaiting_payment", assigned_technician_id: quoteRow.technician_id })
    .eq("id", job_id);
  ...
  try {
    ...
    if (resolved.hasPin && resolved.lat != null && resolved.lng != null) {
      await shortenUrl(`https://maps.google.com/?q=${resolved.lat},${resolved.lng}`, {
        kind: "tech_live_location",
        job_id,
      });
    }
  } catch (e) {
  ...
  await sendMessage(jobRow.customer_phone, customerBody, "whatsapp", job_id);

  return { ok: true, price: Number(mergedPrice) };
}
