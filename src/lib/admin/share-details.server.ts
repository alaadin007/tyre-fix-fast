// Ported from the admin-share-details edge function.
// Same WhatsApp messages to customer + technician, same DB updates.
...
export async function runAdminShareDetails(input: {
  job_id: string;
}): Promise<{ ok: true }> {
  const { job_id } = input;
  ...
  const { data: job } = await supabase
    .from("jobs")
    .select(
      "id, customer_name, customer_phone, postcode, assigned_technician_id, vehicle_reg, affected_wheels, issue_type, issue_description, damage_summary, lat, lng",
    )
    .eq("id", job_id)
    .maybeSingle();
  ...
  const wheels =
    Array.isArray(job.affected_wheels) && job.affected_wheels.length
  ...
  try {
    ...
    if (resolved.hasPin && resolved.lat != null && resolved.lng != null) {
      techLocLink = await shortenUrl(`https://maps.google.com/?q=${resolved.lat},${resolved.lng}`, {
  ...
  if (job.customer_phone) {
    ...
    await sendMessage(job.customer_phone, customerMsg, "whatsapp", job_id);
  }
  ...
  const techMsg = [
    ...
    `━━━━━━━━━━━━━━━`,
    `Issue:  ${issue}`,
    ...(job.issue_description && job.issue_description.trim()
      ? [`📝 Customer description: ${job.issue_description.trim()}`]
      : []),
    `Reg:    ${vehicleReg}`,
    `Wheels: ${wheels}`,
    ``,
  ...
  await sendMessage(tech.phone, techMsg, "whatsapp", job_id);

  await supabase
    .from("jobs")
    .update({ status: "in_progress", assignment_status: "details_sent" })
    .eq("id", job_id);
